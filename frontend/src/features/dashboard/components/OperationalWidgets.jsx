...
export function EquipmentHealthMonitoring({
  ...
  return <Panel title="Equipment Health Monitoring" subtitle="Live asset condition, running exposure, criticality, and maintenance readiness." language={language}>
      <div className="overflow-auto">
        <table className="min-w-[1080px] w-full border-collapse text-sm">
          <thead className="bg-slate-100 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              <th className="border border-slate-200 px-3 py-3 text-left">{t("Asset Name")}</th>
              <th className="border border-slate-200 px-3 py-3 text-left">{t("Asset ID")}</th>
              <th className="border border-slate-200 px-3 py-3 text-left">{t("Current Status")}</th>
              <th className="border border-slate-200 px-3 py-3 text-left">{t("Health Percentage")}</th>
              <th className="border border-slate-200 px-3 py-3 text-left">{t("Running Hours")}</th>
              <th className="border border-slate-200 px-3 py-3 text-left">{t("Last Maintenance")}</th>
              <th className="border border-slate-200 px-3 py-3 text-left">{t("Next Maintenance")}</th>
              <th className="border border-slate-200 px-3 py-3 text-left">{t("Criticality Level")}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(asset => {
            ...
            return <tr key={asset.id} className="bg-white hover:bg-slate-50">
                  ...
                  <td className="border border-slate-200 px-3 py-3"><IndustrialStatusBadge status={equipmentIndustrialStatus(asset)} language={language} /></td>
                  ...
                  <td className="border border-slate-200 px-3 py-3"><CriticalityBadge value={asset.criticality} language={language} /></td>
                </tr>;
          })}
            {!rows.length ? <tr><td colSpan={8} className="border border-slate-200 px-3 py-8 text-center text-slate-500">{t("No assets available for health monitoring.")}</td></tr> : null}
          </tbody>
        </table>
      </div>
    </Panel>;
...
export function SiteStatusOverview({
  ...
  return <Panel title="Site Status Overview" subtitle="Company sites, asset concentration, active faults, online technicians, and operational percentage." language={language}>
      <div className="grid gap-4 xl:grid-cols-3 md:grid-cols-2">
        ...
        return <div key={site.id} className="group rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-md">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-500">{t("Site")}</p>
              ...
              <div className="mt-5 grid grid-cols-3 gap-3">
                <SiteMiniStat label="Assets" value={siteAssets.length} danger={false} language={language} />
                <SiteMiniStat label="Faults" value={activeFaults} danger={activeFaults > 0} language={language} />
                <SiteMiniStat label="Techs" value={activeTechnicians.length} danger={false} language={language} />
              </div>
              <div className="mt-5">
              ...
              <div className="mt-5 rounded-lg border border-slate-100 bg-slate-50 p-3">
                <p className="mb-2 text-xs font-black uppercase tracking-[0.14em] text-slate-500">{t("Mini hierarchy")}</p>
                <div className="space-y-1">
                  {siteAssets.slice(0, 4).map(asset => <div key={asset.id} className="flex items-center gap-2 text-xs font-semibold text-slate-600">
                      ...
                      <span className="truncate">{t(asset.asset_level || "Equipment")} / {asset.name}</span>
                    </div>)}
                  {!siteAssets.length ? <p className="text-xs text-slate-500">{t("No linked assets.")}</p> : null}
                </div>
              </div>
            </div>;
      })}
        {!sites.length ? <EmptyState title="No sites" message="Add customers / sites and link assets to display site status." language={language} /> : null}
      </div>
    </Panel>;
}
...
export function PreventiveMaintenanceDashboard({
  ...
  return <Panel title="Preventive Maintenance Section" subtitle="Upcoming PM tasks, overdue work, completion rate, calendar planning, and planned vs unplanned load." language={language}>
      <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="space-y-4">
          <div className="grid gap-3 md:grid-cols-3">
            <PmStat label="Upcoming PM Tasks" value={upcoming.length} tone="blue" language={language} />
            <PmStat label="Overdue PMs" value={overdue.length} tone={overdue.length ? "red" : "green"} language={language} />
            <PmStat label="PM Completion Rate" value={`${completionRate}%`} tone="green" language={language} />
          </div>
          <div className="overflow-auto rounded-xl border border-slate-200">
            <table className="min-w-[760px] w-full text-sm">
              <thead className="bg-slate-100 text-xs uppercase tracking-[0.12em] text-slate-500">
                <tr>
                  <th className="px-3 py-3 text-left">{t("PM Task")}</th>
                  <th className="px-3 py-3 text-left">{t("Asset")}</th>
                  <th className="px-3 py-3 text-left">{t("Remaining Hours")}</th>
              ...
              <tbody>
                ...
                {!pmTasks.length ? <tr><td colSpan={4} className="px-3 py-8 text-center text-slate-500">{t("No PM tasks configured.")}</td></tr> : null}
              </tbody>
            </table>
          </div>
        </div>
        <div className="space-y-4">
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <p className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-slate-500">{t("Calendar View")}</p>
            <div className="space-y-2">
              {calendarRows.map(task => <div key={task.id} className={`rounded-lg border px-3 py-2 ${isPmOverdue(task) ? "border-red-200 bg-red-50" : "border-slate-200 bg-slate-50"}`}>
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-xs font-bold text-slate-500">{task.next_due_date || t("Hours-based")}</span>
                  </div>
                  <p className="mt-1 text-xs text-slate-500">{task.equipment_name} / {formatScheduleCell(task.hours_until_due)} {t("hrs")}</p>
                </div>)}
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <p className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-slate-500">{t("Planned vs Unplanned")}</p>
            <BarChart data={[{
            label: t("Planned"),
            value: plannedCount,
            color: "bg-blue-600"
...
export function AlertsAlarmsSection({
  ...
  return <Panel title="Alerts & Alarms Section" subtitle="Real-time industrial alarm panel with critical indicators, downtime exposure, and affected equipment." language={language}>
      <div className="mb-4 grid gap-3 md:grid-cols-3">
        <PmStat label="Critical Alarms" value={criticalAlarms.length} tone={criticalAlarms.length ? "red" : "green"} language={language} />
        <PmStat label="Total Alarms" value={alerts.length} tone={alerts.length ? "orange" : "green"} language={language} />
        <PmStat label="Priority Work Orders" value={workOrders.filter(order => ["critical", "high"].includes(order.priority)).length} tone="blue" language={language} />
      </div>
      <div className="overflow-auto rounded-xl border border-slate-200">
        <table className="min-w-[900px] w-full text-sm">
          <thead className="bg-slate-100 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              <th className="px-3 py-3 text-left">{t("Priority")}</th>
              <th className="px-3 py-3 text-left">{t("Fault Type")}</th>
              <th className="px-3 py-3 text-left">{t("Timestamp")}</th>
              <th className="px-3 py-3 text-left">{t("Affected Equipment")}</th>
              <th className="px-3 py-3 text-left">{t("Current Downtime")}</th>
            </tr>
          </thead>
          <tbody>
            ...
            return <tr key={isAlert ? getAlertKey(alarm) : `work-order-${alarm.id || index}`} className={critical ? "bg-red-50" : "bg-white"}>
                  ...
                  <td className="border-t border-slate-200 px-3 py-3 text-slate-600">{alarm.created_at?.slice(0, 16) || alarm.next_maintenance_date || alarm.due_date || t("Live")}</td>
                  <td className="border-t border-slate-200 px-3 py-3 font-black text-slate-950">{asset?.name || alarm.equipment_name || t("Unassigned")}</td>
                  <td className="border-t border-slate-200 px-3 py-3 font-black text-red-600">{alarmDowntime(alarm)}</td>
                </tr>;
          })}
            {!alarmRows.length ? <tr><td colSpan={5} className="px-3 py-8 text-center text-slate-500">{t("No active alarms.")}</td></tr> : null}
          </tbody>
        </table>
      </div>
    </Panel>;
...
export function InventoryMonitoringSection({
  ...
  return <Panel title="Inventory & Spare Parts" subtitle="Stock health, critical spares, purchase pressure, and spare-part usage indicators." language={language}>
      <div className="mb-4 grid gap-3 md:grid-cols-3">
        <PmStat label="Low Stock Items" value={lowStock.length} tone={lowStock.length ? "orange" : "green"} language={language} />
        <PmStat label="Critical Spare Parts" value={criticalParts.length} tone={criticalParts.length ? "red" : "green"} language={language} />
        <PmStat label="Purchase Requests" value={lowStock.length} tone="blue" language={language} />
      </div>
      <div className="space-y-4">
        {mostUsed.map(part => {
        ...
        return <div key={part.id} className="rounded-xl border border-slate-200 bg-white p-4">
              <div className="mb-2 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-500">{part.part_number || t("No part no.")} / {part.category || t("General")}</p>
                </div>
                <StockBadge value={part.stock_alert} language={language} />
              </div>
              <div className="flex items-center gap-3">
                <ProgressBar value={percent} tone={tone} />
                <span className="w-20 text-right text-sm font-black text-slate-700">{stock} {part.unit || t("pcs")}</span>
              </div>
            </div>;
      })}
        {!inventory.length ? <EmptyState title="No inventory" message="Add spare parts to monitor stock and purchase requests." language={language} /> : null}
      </div>
      <div className="mt-5 rounded-xl border border-slate-200 bg-white p-4">
        <p className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-slate-500">{t("Inventory Chart")}</p>
        <BarChart data={[{
        label: t("OK"),
...
export function TechnicianPerformanceSection({
  ...
  return <Panel title="Technician Performance" subtitle="Active resources, completion throughput, response speed, repair duration, and efficiency ranking." language={language}>
      <div className="mb-4 grid gap-3 md:grid-cols-3">
        <PmStat label="Active Technicians" value={activeTechnicians.length} tone="green" language={language} />
        <PmStat label="Tasks Completed" value={workOrders.filter(order => order.status === "completed").length} tone="blue" language={language} />
        <PmStat label="Avg Response Time" value={`${Math.max(15, 48 - activeTechnicians.length * 3)} ${t("min")}`} tone="orange" language={language} />
      </div>
      <div className="overflow-auto rounded-xl border border-slate-200">
        <table className="min-w-[760px] w-full text-sm">
          <thead className="bg-slate-100 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              <th className="px-3 py-3 text-left">{t("Rank")}</th>
              <th className="px-3 py-3 text-left">{t("Technician")}</th>
              <th className="px-3 py-3 text-left">{t("Tasks Completed")}</th>
              <th className="px-3 py-3 text-left">{t("Response Time")}</th>
              <th className="px-3 py-3 text-left">{t("Avg Repair Duration")}</th>
              <th className="px-3 py-3 text-left">{t("Efficiency")}</th>
            </tr>
          </thead>
          <tbody>
            {rankings.map((engineer, index) => <tr key={engineer.id} className="bg-white hover:bg-slate-50">
                ...
                <td className="border-t border-slate-200 px-3 py-3">
                  ...
                  <p className="text-xs font-semibold text-slate-500">{engineer.specialty || t("Maintenance")}</p>
                </td>
            ...
            {!rankings.length ? <tr><td colSpan={6} className="px-3 py-8 text-center text-slate-500">{t("No technicians available.")}</td></tr> : null}
          </tbody>
        </table>
      </div>
      <div className="mt-5 rounded-xl border border-slate-200 bg-white p-4">
        <p className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-slate-500">{t("Performance Chart")}</p>
