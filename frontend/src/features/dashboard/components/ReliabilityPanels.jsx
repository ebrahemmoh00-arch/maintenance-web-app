...
export function DashboardBottomRow({
  ...
  return <div className="grid gap-6 xl:grid-cols-2">
      ...
      <TopDowntimeAssetsPanel rows={metrics.topDowntimeAssets} language={language} />
      <AssetHealthRankingPanel rows={metrics.assetHealthRanking} average={metrics.assetHealthAverage} language={language} />
    </div>;
}
...
export function CostRanking({
  ...
  return <div className="rounded-lg border border-slate-200 bg-white p-3">
      <p className="mb-2 text-xs font-black uppercase tracking-[0.12em] text-slate-500">{t(title)}</p>
      <div className="space-y-2">
        ...
        {!rows.length ? <p className="text-sm font-semibold text-slate-400">{t("No cost records")}</p> : null}
      </div>
    </div>;
}
...
export function CriticalEquipmentStatusPanel({
  ...
  const t = text => tr(language, text);
  return <Panel title="Critical Equipment Status" subtitle="Critical assets grouped by running, maintenance, breakdown, and out-of-service states." language={language}>
      <div className="mb-4 grid gap-2 sm:grid-cols-2">
        {siteSummary.slice(0, 4).map(site => <div key={site.name} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
            ...
            <p className="text-xs font-semibold text-slate-500">{site.running}/{site.assets} {t("running")} / {site.breakdown} {t("breakdown")}</p>
          </div>)}
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        {rows.slice(0, 8).map(asset => <div key={asset.id} className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="mb-3 flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs font-semibold text-slate-500">{asset.customer_name || asset.location || t("Unassigned site")}</p>
              </div>
              <IndustrialStatusBadge status={asset.statusLabel} language={language} />
            </div>
        ...
        {!rows.length ? <EmptyState title="No critical equipment" message="Mark assets as High or Critical to monitor operational status." language={language} /> : null}
      </div>
    </Panel>;
}
...
export function OverduePmTasksPanel({
  ...
  const t = text => tr(language, text);
  return <Panel title="Overdue Preventive Maintenance Tasks" subtitle="Tasks that passed due date or exceeded scheduled service hours." language={language}>
      {rows.length ? <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-black text-red-700">
          {t("Critical Alert")}: {rows.length} {t("preventive maintenance tasks are overdue.")}
        </div> : null}
      <div className="overflow-auto rounded-xl border border-slate-200">
        <table className="min-w-[760px] w-full text-sm">
          <thead className="bg-slate-100 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              <th className="px-3 py-3 text-left">{t("PM Number")}</th>
              <th className="px-3 py-3 text-left">{t("Asset")}</th>
              <th className="px-3 py-3 text-left">{t("Required Maintenance")}</th>
              <th className="px-3 py-3 text-left">{t("Site")}</th>
              <th className="px-3 py-3 text-left">{t("Due Date")}</th>
              <th className="px-3 py-3 text-left">{t("Days Overdue")}</th>
            </tr>
          </thead>
          <tbody>
...
export function TopDowntimeAssetsPanel({
  ...
  return <Panel title="Top 10 Assets by Downtime" subtitle="Equipment with the highest downtime exposure in the selected period." language={language}>
...
export function AssetHealthRankingPanel({
  ...
  const t = text => tr(language, text);
  return <Panel title="Asset Health Ranking" subtitle="Calculated health score based on breakdown frequency, MTTR, MTBF, availability, and PM exposure." language={language}>
      <div className="mb-5 flex flex-wrap items-center gap-5">
        <GaugeChart value={average} language={language} />
        <div>
          <p className="text-sm font-black uppercase tracking-[0.14em] text-slate-500">{t("Health Categories")}</p>
          <p className="mt-2 text-sm font-semibold text-slate-600">{t("90-100 Excellent / 75-89 Good / 60-74 Fair / Below 60 Poor")}</p>
        </div>
      </div>
      <div className="overflow-auto rounded-xl border border-slate-200">
        <table className="min-w-[720px] w-full text-sm">
          <thead className="bg-slate-100 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
              <th className="px-3 py-3 text-left">{t("Asset")}</th>
              <th className="px-3 py-3 text-left">{t("Site")}</th>
              <th className="px-3 py-3 text-left">{t("Health")}</th>
          ...
          <tbody>
            {rows.slice(0, 10).map(asset => <tr key={asset.id} className="bg-white hover:bg-slate-50">
                ...
                <td className="border-t border-slate-200 px-3 py-3 font-bold text-slate-700">{t(asset.category)}</td>
              </tr>)}
            {!rows.length ? <tr><td colSpan={4} className="px-3 py-8 text-center text-slate-500">{t("No asset health records.")}</td></tr> : null}
          </tbody>
        </table>
      </div>
    </Panel>;
...
export function GaugeChart({
  ...
  return <svg viewBox="0 0 42 42" className="h-32 w-32 shrink-0">
      ...
      <text x="21" y="25" textAnchor="middle" className="fill-slate-500 text-[0.18rem] uppercase">{tr(language, "Health")}</text>
    </svg>;
}
...
export function AssetReliabilityPanel({
  ...
  const t = text => tr(language, text);
  return <Panel title="Asset Reliability" subtitle="Bad actors, downtime tracking, MTTR, and MTBF indicators for production reliability." language={language}>
      <div className="mb-4 grid gap-3 md:grid-cols-4">
        <PmStat label="Bad Actors" value={reliability.badActors.length} tone={reliability.badActors.length ? "orange" : "green"} language={language} />
        <PmStat label="Downtime Hours" value={reliability.downtimeLabel} tone={reliability.totalDowntimeHours > 0 ? "red" : "green"} language={language} />
        <PmStat label="MTTR" value={reliability.mttrLabel} tone={reliability.mttrHours > 4 ? "orange" : "green"} language={language} />
        <PmStat label="MTBF" value={reliability.mtbfLabel} tone={reliability.mtbfHours < 100 && reliability.mtbfHours > 0 ? "orange" : "blue"} language={language} />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_1.3fr]">
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-black text-slate-950">{t("Bad Actors")}</h3>
              <p className="text-xs font-semibold text-slate-500">{t("Top 5 assets causing production downtime.")}</p>
            </div>
            ...
            {reliability.badActors.map((asset, index) => <div key={asset.id || asset.name} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs font-semibold text-slate-500">{asset.faults} {t("faults")} / {asset.downtimeLabel} {t("downtime")}</p>
                  </div>
                  <span className="rounded bg-red-50 px-2 py-1 text-xs font-black text-red-700">{asset.impactScore}</span>
                </div>
              </div>)}
            {!reliability.badActors.length ? <EmptyState title={t("No data")} message="No breakdown-related work orders found." language={language} /> : null}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-black text-slate-950">{t("Downtime Tracking")}</h3>
              <p className="text-xs font-semibold text-slate-500">{t("Production line downtime hours by date.")}</p>
            </div>
