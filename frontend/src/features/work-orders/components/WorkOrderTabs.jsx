...
export function WorkOrderQuickInfo({
  ...
  return <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h3 className="text-sm font-black text-slate-950">{t("Quick Information")}</h3>
        <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-black text-blue-700">{t("CMMS Summary")}</span>
      </div>
...
export function WorkOrderKpi({
  ...
  return <div className={`rounded-xl p-3 ring-1 ${tones[tone] || tones.blue}`}>
      <p className="text-xs font-black uppercase tracking-[0.12em] opacity-80">{t(label)}</p>
...
export function WorkOrderOverviewTab({
  ...
  return <div className="grid gap-5 2xl:grid-cols-[1fr_320px]">
      <div className="space-y-5">
        <ModernTextArea label="Work Description" value={form.task_description} onChange={value => update("task_description", value)} rows={5} language={language} />
        <div className="rounded-2xl bg-slate-50 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">{t("Current Status")}</p>
            ...
            <select className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-black outline-none focus:border-blue-500" value={form.status || ""} onChange={event => updateStatus ? updateStatus(event.target.value) : update("status", event.target.value)}>
              {["pending", "new", "assigned", "accepted", "in_progress", "on_hold", "waiting_for_parts", "completed", "pending_supervisor_review", "approved", "closed", "cancelled", "rejected", "overdue"].map(option => <option key={option} value={option}>{t(option.replaceAll("_", " "))}</option>)}
            </select>
          </div>
          <div className="mt-5">
            <WorkOrderLifecycleProgress status={status} language={language} />
          </div>
        </div>
        <div className="grid gap-4 lg:grid-cols-2">
          <CompactPhotoUploader title={t("Before Maintenance Photos")} photos={photosBefore || []} onChange={photos => updatePhotos("before_photos", photos)} language={language} />
          <CompactPhotoUploader title={t("After Maintenance Photos")} photos={photosAfter || []} onChange={photos => updatePhotos("after_photos", photos)} language={language} />
        </div>
      </div>
      <div className="space-y-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          <h3 className="text-sm font-black text-slate-950">{t("Summary")}</h3>
          <div className="mt-4 space-y-3">
            <SummaryLine label="Asset" value={selectedEquipment?.name || "-"} language={language} />
            <SummaryLine label="Location" value={selectedCustomer?.name || selectedEquipment?.location || "-"} language={language} />
            <SummaryLine label="Serial Number" value={selectedEquipment?.serial_number || form.serial_number || "-"} language={language} />
            <SummaryLine label="Runtime" value={`${Number(selectedEquipment?.current_hours ?? form.service_hours ?? 0).toLocaleString()} h`} language={language} />
          </div>
        </div>
      </div>
    </div>;
      ...
      <div className="flex min-w-[760px] items-center">
        ...
        return <Fragment key={step}>
              <div className="flex flex-col items-center gap-2">
                ...
                <span className={`whitespace-nowrap text-[11px] font-black uppercase tracking-[0.08em] ${isActive ? "text-blue-700" : "text-slate-500"}`}>{t(step.replaceAll("_", " "))}</span>
              </div>
...
export function WorkOrderChecklistTab({
  ...
  const tasks = [form.task_description || t("Work description confirmed"), form.requirements || t("Necessary requirements checked"), t("Safety and QHSE requirements reviewed"), t("Photos and completion evidence prepared")];
  return <div className="space-y-5">
      <div className="rounded-2xl bg-slate-50 p-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-black text-slate-950">{t("Checklist Completion")}</h3>
            <p className="mt-1 text-sm font-semibold text-slate-500">{t("Work order completion requires 100% checklist confirmation.")}</p>
          </div>
      ...
      <div className="space-y-3">
        {tasks.map((task, index) => <div key={`${task}-${index}`} className="grid gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 md:grid-cols-[40px_1fr_180px_1fr]">
            ...
            <select className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold outline-none focus:border-blue-500" defaultValue="">
              ...
              <option>{t("OK")}</option>
              <option>{t("Needs action")}</option>
              <option>{t("N/A")}</option>
            </select>
            <input className="rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-blue-500" placeholder={t("Comments")} />
          </div>)}
      </div>
    </div>;
}
...
export function WorkOrderLaborTab({
  ...
  return <div className="grid gap-5 lg:grid-cols-2">
      <div className="space-y-4">
        <ModernField label="Technician" value={seniorTeamTechnician?.name || "-"} readOnly language={language} />
        <div className="grid gap-3 sm:grid-cols-2">
          <ModernTimeField label="Start Time" value={form.start_time} onChange={value => update("start_time", value)} language={language} />
          <ModernTimeField label="Finish Time" value={form.finished_time} onChange={value => update("finished_time", value)} language={language} />
          <ModernDateField label="Start Date" value={form.start_date} onChange={value => update("start_date", value)} language={language} />
          <ModernDateField label="Finish Date" value={form.finished_date} onChange={value => update("finished_date", value)} language={language} />
        </div>
        <div className="grid gap-3 sm:grid-cols-3">
          <WorkOrderKpi label="Break Time" value="0:00" tone="slate" language={language} />
          <WorkOrderKpi label="Working Hours" value={duration} language={language} />
          <WorkOrderKpi label="Downtime" value={duration} tone={duration === "0:00" ? "slate" : "red"} language={language} />
        </div>
      </div>
      <div className="space-y-4">
        <ModernField label="Runtime Reading" value={draft.runtime_reading} onChange={value => updateDraft("runtime_reading", value)} placeholder={String(selectedEquipment?.current_hours || form.service_hours || 0)} type="number" language={language} />
        <ModernTextArea label="Completion Notes" value={draft.completion_notes} onChange={value => updateDraft("completion_notes", value)} rows={4} language={language} />
        <ModernTextArea label="Failure Cause" value={draft.reason} onChange={value => updateDraft("reason", value)} rows={3} language={language} />
      </div>
    </div>;
}
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[1080px] w-full text-left text-sm">
            <thead className="bg-slate-50 text-xs font-black uppercase tracking-[0.12em] text-slate-500">
              <tr>
                {["Part", "Code", "Warehouse", "Available", "Reserved", "Quantity Used", "Unit Cost", "Total", ""].map(header => <th key={header || "actions"} className="px-4 py-3">{header ? t(header) : ""}</th>)}
              </tr>
            </thead>
                ...
                return <tr key={index} className="hover:bg-slate-50">
                  <td className="px-4 py-3">
                    <select className="w-full min-w-[240px] rounded-lg border border-slate-200 px-3 py-2 font-bold outline-none focus:border-blue-500" value={item.inventory_item_id || inventoryItem.id || ""} onChange={event => chooseInventoryPart(index, event.target.value)}>
                      <option value="">{t("Select spare part")}</option>
                      {inventory.map(part => <option key={part.id} value={part.id}>{part.name} ({Number(part.stock_quantity || 0)} {part.unit || t("pcs")})</option>)}
                    </select>
                  </td>
                  <td className="px-4 py-3 text-slate-600">{item.part_number || item.code || inventoryItem.part_number || "-"}</td>
                  <td className="px-4 py-3 text-slate-600">{item.warehouse || inventoryItem.location || t("Inventory")}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <button type="button" onClick={() => onRemove?.(index)} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-red-100 bg-white text-red-600 hover:bg-red-50" title={t("Remove part from work order")}>
                      <Trash2 className="h-4 w-4" />
                    </button>
                  </td>
                </tr>;
              })}
              {!items.length ? <tr><td colSpan={9} className="px-4 py-8 text-center text-sm font-semibold text-slate-500">{t("No spare parts selected.")}</td></tr> : null}
            </tbody>
          </table>
        </div>
      </div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <button type="button" onClick={onAdd} className="rounded-xl bg-blue-700 px-4 py-2 text-sm font-black text-white hover:bg-blue-800">{t("Add Part")}</button>
      ...
      {rows.map(([label, date, user, description], index) => <div key={`${label}-${index}`} className="grid grid-cols-[16px_1fr] gap-3">
          ...
          <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-sm font-black text-slate-950">{t(String(label || "").replaceAll("_", " "))}</p>
              <span className="text-xs font-bold text-slate-500">{formatLifecycleDate(date)}</span>
            </div>
            <p className="mt-1 text-sm font-semibold text-slate-600">{description || t("Action recorded")}</p>
            {user ? <p className="mt-2 text-xs font-black uppercase tracking-[0.12em] text-blue-700">{user}</p> : null}
          </div>
        </div>)}
      {!rows.length ? <EmptyState title="No history yet" message="Actions will appear here after the work order is saved and progressed." language={language} /> : null}
    </div>;
}
...
export function WorkOrderNotesTab({
  ...
  return <div className="space-y-5">
      <ModernTextArea label="Recommendation / Comment" value={form.recommendation} onChange={value => update("recommendation", value)} rows={4} language={language} />
      <div className="space-y-3">
        {notes.map(([title, comment], index) => <div key={`${title}-${index}`} className="flex gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <p className="text-sm font-black text-slate-950">{selectedEngineer?.name || t("System User")}</p>
                <span className="text-xs font-bold text-slate-400">{t("Now")}</span>
              </div>
              <p className="mt-1 text-sm font-semibold leading-6 text-slate-600">{comment}</p>
            </div>
          </div>)}
        {!notes.length ? <EmptyState title="No comments" message="Write a note above to document field observations." language={language} /> : null}
      </div>
    </div>;
}
  ...
  return <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs font-bold text-slate-500">{photos.length} {t("images")}</p>
        </div>
      ...
      <div className="mt-4 grid grid-cols-4 gap-2">
        ...
        {!photos.length ? <div className="col-span-4 rounded-xl border border-dashed border-slate-300 bg-white py-5 text-center text-xs font-bold text-slate-500">{t("Drag & drop area / no photos")}</div> : null}
      </div>
    </div>;
}
...
export function ModernField({
  ...
  return <label className="block">
      ...
      <input type={type} readOnly={readOnly} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm font-bold outline-none focus:border-blue-500 read-only:bg-slate-50" value={value || ""} placeholder={placeholder ? t(placeholder) : ""} onChange={event => onChange?.(event.target.value)} />
    </label>;
}
